...
//			file_put_contents( __DIR__ . "/processResults.txt", var_export( $resultsArray, true ), FILE_APPEND );
...
//		var_dump($results);
//		file_put_contents(__DIR__ . "/get_allmembers_as_json.txt", var_export($results, true), FILE_APPEND);
...
//			file_put_contents(__DIR__ . "/get_collectors_json.txt", var_export($short_code_stream, true), FILE_APPEND);
...
//				file_put_contents(__DIR__ . "/get_members_by_LANumber.txt", var_export($results, true), FILE_APPEND);
		...
		public static function do_parseAndInsertCSV() {
			...
			$csvfile = $CSV_PATH . "repak-local-authority-listB.csv";
//			$csvfile = $CSV_PATH . "my_list.csv";

//	  file_put_contents(__DIR__ . "/csvfile.txt", var_export($csvfile, true), FILE_APPEND);
...
//	  file_put_contents(__DIR__ . "/csvfile.txt", var_export($file, true), FILE_APPEND);
...
//					file_put_contents(__DIR__ . "/var_line_text.txt", var_export($line_of_text, true), FILE_APPEND);
					if ( $lotCounter > 0 ) {
						...
						try {
							$values = "("
							          ...
							          . ",'" . str_replace( "'", "\'", trim( $line_of_text[12] ) ) . "'"
							          . ")";
						} catch ( Exception $ex ) {
...
//							file_put_contents(__DIR__ . "/values.txt", var_export($values, true), FILE_APPEND);
...
//      file_put_contents(__DIR__ . "/rename-table.txt", var_export($renameQuery, true), FILE_APPEND);
...
//				file_put_contents( __DIR__ . "/activate_createTable.txt", var_export( $sql, true ), FILE_APPEND );
		...
		//Add a new time interval for cron tasks
