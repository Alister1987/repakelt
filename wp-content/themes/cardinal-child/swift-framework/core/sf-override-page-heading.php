...
/* PAGE HEADING
================================================== */
if ( ! function_exists( 'custom_sf_page_heading' ) ) {
	function custom_sf_page_heading() {
		...
		if ( isset( $sf_options['minimal_checkout'] ) ) {
			if ( function_exists( 'is_checkout' ) && is_checkout() ) {
				...
				if ( $sf_options['minimal_checkout'] ) { ?>
                    ...
                    <div class="minimal-checkout-return container"><a
                                href="<?php echo esc_url( $woocommerce->cart->get_cart_url() ); ?>"><?php _e( "Return to cart", "swiftframework" ); ?></a>
                    </div>
			...
			<?php if ( $page_title_style == "fancy" || $page_title_style == "fancy-tabbed" ) { ?>
				...
				<?php if ( $fancy_title_image_url != "" ) {
                    ...
                    <span class="media-overlay"
                          style="background-color:<?php echo $bg_color_title; ?>;opacity:<?php echo $bg_opacity_title; ?>;"></span>
                ...
                <div class="container">
                        ...
                        <div class="post-nav">
							<?php if ( isset( $portfolio_page ) ) { ?>
                                <div class="view-all"><a
                                            href="<?php echo get_permalink( $portfolio_page ); ?>"><?php echo $index_icon; ?></a>
                                </div>
                    ...
                    <div class="heading-text">
						...
						<?php } else if ( is_search() ) { ?>
							...
							$s                = get_search_query();
							$key              = esc_html( $s );
							if ( isset( $search_post_type ) ) {
								$allSearchQuery   = "s=$key&post_type=$search_post_type&showposts=-1";
							}
							else{
								$allSearchQuery   = "s=$key&showposts=-1";
                            }

							$allSearch        = new WP_Query( $allSearchQuery );
					...
					<?php if ( is_singular( 'portfolio' ) && ! ( sf_theme_opts_name() == "sf_joyn_options" && $pagination_style == "fs-arrow" ) && sf_theme_opts_name() != "sf_uplift_options" ) { ?>
						...
						<?php if ( sf_theme_opts_name() == "sf_atelier_options" && isset( $portfolio_page ) ) { ?>
                            <div class="view-all" <?php echo $article_heading_text; ?>><a
                                        href="<?php echo get_permalink( $portfolio_page ); ?>"><?php echo $index_icon; ?></a>
                            </div>
						<?php } ?>
                        <div class="prev-item" <?php echo $article_heading_text; ?>><?php next_post_link( '%link', $prev_icon, $enable_category_navigation, '', 'portfolio-category' ); ?></div>
	...
	add_action( 'sf_main_container_start', 'custom_sf_page_heading', 30 );
}
